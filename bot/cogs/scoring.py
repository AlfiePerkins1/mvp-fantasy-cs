...
class Scoring(commands.Cog):
    ...
    # @scoring.command(name="update", description="(Admin) Fetch new games and recompute weekly points")
    # @app_commands.checks.has_permissions(administrator=True)
    # @app_commands.describe(member="Only update this member (optional)")
    # async def scoring_update(self, interaction: discord.Interaction, member: Optional[discord.User] = None):
    ...
    #     ruleset_id = 1
    #
    #     # 1) Build the user set (DB user_ids) we need to process
    #     async with SessionLocal() as session:
    #         async with session.begin():
    #             if member is not None:
    #                 db_user = await get_or_create_user(session, member.id)
    #                 user_ids = [db_user.id]
    #                 targets = [(db_user.id, member.id)]
    #             else:
    #                 # everyone on any team in this guild
    #                 user_ids = await _guild_roster_db_user_ids(session, guild_id)
    #                 # map db_user_id -> discord_id for ingest step
    #                 targets = []
    #                 for db_uid in user_ids:
    #                     u = await session.scalar(select(User).where(User.id == db_uid))
    #                     if u:
    #                         targets.append((db_uid, int(u.discord_id)))
    #
    #     if not targets:
    ...
    #     updated_total = 0
    #
    #     # 2) Ingest + aggregate + score for each target
    #     async with SessionLocal() as session:
    #         async with session.begin():
    #             for db_user_id, discord_id in targets:
    #                 # ingest any new matches for this user (idempotent)
    #                 try:
    #                     stats = await ingest_user_recent_matches(session, discord_id=discord_id, limit=100)
    #                     ingested_total += stats.get("inserted", 0)
    #                 except Exception as e:
    #                     print(f"[scoring.update] ingest failed for {discord_id}: {e}")
    #
    #                 # aggregate this week from DB
    #                 agg = await aggregate_week_from_db(session, user_id=db_user_id, week_start_utc=week_start_utc)
    #                 # score + upsert weekly_points
    #                 bd = breakdown_from_agg(agg)
    ...
    #         f"Fetched **{ingested_total}** new games and recomputed **{updated_total}** weekly snapshot(s){who}.",
    #         ephemeral=True,
    #     )
