...
class User(Base):
    ...
    __table_args__ = (
        UniqueConstraint("discord_id", "discord_guild_id", name="uq_user_per_guild"),
        Index("ix_users_discord_guild", "discord_id", "discord_guild_id"),
    )
...
class Player(Base):
    ...
    user_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"), index=True, unique=True, nullable=True)
    user: Mapped["User"] = relationship(back_populates="player", uselist=False)
